...
void Link::Update(float deltaTime)
{
	...
	if (displacement.x > 0.0f)
	{
		...
		if (TileMap::Get().IsCollidingWithGrass(rightEdge))
		{
			probability = X::RandomFloat(0.0f, 100.0f);
		}

		if (TileMap::Get().IsCollidingWithDestination(rightEdge))
		{
			number++;
			result = true;
		}
	...
	if (displacement.x < 0.0f)
	{
		...
		if (TileMap::Get().IsCollidingWithGrass(leftEdge))
		{
			probability = X::RandomFloat(0.0f, 100.0f);
		}

		if (TileMap::Get().IsCollidingWithDestination(leftEdge))
		{
			number++;
			result = true;
		}
	...
	if (displacement.y > 0.0f)
	{
		...
		if (TileMap::Get().IsCollidingWithGrass(bottomEdge))
		{
			probability = X::RandomFloat(0.0f, 100.0f);
		}

		if (TileMap::Get().IsCollidingWithDestination(bottomEdge))
		{
			number++;
			result = true;
		}
	...
	if (displacement.y < 0.0f)
	{
		...
		if (TileMap::Get().IsCollidingWithGrass(topEdge))
		{
			probability = X::RandomFloat(0.0f, 100.0f);
		}

		if (TileMap::Get().IsCollidingWithDestination(topEdge))
		{
			number++;
			result = true;
		}
